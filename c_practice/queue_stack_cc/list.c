...
/*
 *  This function removes the first node of a list and frees that node
 *  , the next node after the now deleted one  will be the new first node
 *  param : struct list* list
 *  
 */
void rm_front(struct list *list){
    assert(list);
    struct node* next_node = list->head->next,*curr_node = list->head;
    list->head = next_node;
    free(curr_node);
    return;
...
/*
 *  This function checks whether a list is empty or not
 *  param : struct list* list
 *  output : if empty then 1 will be returned, if not empty 0 is returned
 * */
int list_empty(struct list* list){
    int empty_or_not = list->head == NULL ? 1:0;
    return empty_or_not;
