...
int queue_isempty(struct queue* queue) {
    assert(queue);
// this wont work as intended, make a function in dynarray.c|int empty_or_not = queue->array->size == NULL ? 1:0; 
    int empty_or_not = array_isempty(queue->array); 
    return empty_or_not;
}
...
//----------------------------------------------------------------------------------------------------
void recieved(struct queue* queue, void* call){
    if (dynarray_get(queue->array,(dynarray_size(queue->array)-1)) == call){
        printf("The call has been successfully added to the queue!");
    }else{
        printf("The call has NOT been successfully added to the queue!");
    }
    return;
}
