...
{
    struct list* new_lst = malloc(sizeof(struct list*));
    new_lst->head = NULL;
...
void list_insert(struct list* list, void* val)
{
    ...
    temp->val = val;//temp has val of "val"
    temp->next = list->head;//even if list is null, it doesn't matter because 
    //temp->next would then be pointing to null,
...
void list_insert_end(struct list* list, void* val)
{
    ...
    temp->next = NULL;
    if(list->head != NULL){
        struct node* new_lst =  list->head;
        while(new_lst->next != NULL){
            //iterates through all nodes until it reaches its last node
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    if(temp != NULL && cmp(temp->val,val) == 0){//checks if head is equal
        //to val
        list->head = temp->next;
        free(temp);
        return;
    }
    while(temp != NULL && cmp(temp->val,val) != 0){    

        prev = temp;
        temp = temp->next;

    }if(temp == NULL){//returns if there the val is not in the list
            return;
    }else{
        prev->next = temp->next;//skips node that will be rm||  =->=X=->=->
        free(temp);//rm node 
        return;
    }
    return;
...
void list_remove_end(struct list* list){
    struct node* temp = list->head, *prev;
    if(temp == NULL){//if list is empty return
        return;
    }
    if(temp->next == NULL){//if list only has one node, rm it
        free(temp);
        list->head = NULL;
        return;
    }

    while(temp->next != NULL){//if list has more than one node
        // iterate to the last node and rm it
        prev = temp;
        temp = temp->next;
    }    
    prev->next = NULL;
...
/*
 * Thisfunction should return the position (i.e. the 0-based "index") of the
 ...
 * cmp - pointer to a function that can be passed two void* values from
 *     to compare them for equality, as described above.  If the two values
 *     passed re to be considered equal, this function should return 0.
 *     Otherwise, it should return a non-zero value.
 *
 * Return:
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    if(temp != NULL && cmp(temp->val,val) == 0){
        //checks if head is equal to val
        return idx;
    }
    while(temp != NULL){    
        ...
        printf("list does not contain value you are looking for\n");    
        return -1;
    }
}
...
{
    struct node *curr = list->head,*next = NULL, *prev = NULL;
    while(curr != NULL){
        next = curr->next;
