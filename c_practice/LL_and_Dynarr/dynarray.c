 ...
 *
 * Name:David Velasco Perez(Github ID: Dozaay)
...
{
    int cap =  2;
    struct dynarray* new_arr = malloc(sizeof(struct dynarray*));
    new_arr->capacity = cap;
    new_arr->size = 0;
    new_arr->data = malloc(cap*sizeof(void*)); 
...
void dynarray_insert(struct dynarray* da, void* val)
{
    ...
    //case 2
        ...
        cap *= 2;
        da->data = realloc(da->data,cap*sizeof(void*)); 
...
void dynarray_remove(struct dynarray* da, int idx)
{
    int size = dynarray_size(da);
    if(idx > size||idx < 0){
        printf("index does not exist within than number of elements in array");
        return;
    }
