...
void list_remove(struct list* list, void* val, int (*convert)(void*)) {
  ...
  while (curr) {
     ...
     * the list.
     */
    if (convert(curr->val) == convert(val)) {
      if (prev) {
        prev->next = curr->next;
      } else {
        list->head = curr->next;
 ...
 * Description: This function will return the size of a list
 * Param: 
 *      list - the list we will be returning the size of
 * */
int list_size(struct list* list){
    int size = 0;
    struct node* curr = list->head;
    // will loop through the list until it next points to null
    // size is incremented at each loop
    while(curr){
            size++;
            curr = curr->next;
    }
...
/*
 * Function Name: head_get
 * Description: this function will return the head of a list,
 *              for when head can't be accessed directly.
 * Param:
 *      list - the list we want head from(he he he)
 */
void* head_get(struct list* list){
    return list->head;
}

/*
 * Function Name: list_insert_key
 * Description: this function inserts a value into the list. 
 *              this function checks if a key already exist for this value, 
 *              if does it updates the value, else it inserts it                 
 * Params: 
 *      list - the linked where the value will be inserted into 
 *      val - the value to be inserted 
 *      key - the key associated with the value
 *      convert - will be used to check the hash code of a value/key 
 * */
void list_insert_key(struct list *list, void* val, void* key, int(*convert)(void*)){
    struct node *curr = list->head;
    
    while(curr){
        // checks if there is key that already exist at this key
        // updates if so
        if (convert(curr->val) == convert(key)){
            curr->val = val;
            return;
        }
        curr = curr->next;
    }
    // if it passes all the checks it is inserted into the list
 ...
 * Description: this function will return the next node of a list, 
 *              for when the next node can't be accessed directly
 * Param:
 *      node - the node we want to get the next node from
 */
...
/*
 * Function Name: node_val
 * Description: this function will return the current value of a node
 *              , for when the value can't be accessed directly
 * Param:
 *      node - the node we want the current value of
 */
void* node_val(void* node){
    return ((struct node*)node)->val;
